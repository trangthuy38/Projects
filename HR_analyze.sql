...
--Attrition by Job Involvement and Performance Rating
with a as (
			select JobInvolvement, PerformanceRating, count(Attrition) as Attrition
		    from hr_rmp
			where Attrition = 1
			group by JobInvolvement, PerformanceRating),
b as (
			select JobInvolvement, PerformanceRating, count(Attrition) as Total_Employees
		    from hr_rmp
			group by JobInvolvement, PerformanceRating)
select b.JobInvolvement, b.PerformanceRating, b.Total_Employees, a.Attrition, CAST(ROUND((a.Attrition*1.0/ b.Total_Employees) * 100, 2) AS DECIMAL(9, 2)) as Attrition_Rate
from a 
inner join b 
on a.JobInvolvement = b.JobInvolvement and a.PerformanceRating=b.PerformanceRating 
